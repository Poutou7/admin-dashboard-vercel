  ...
  <v-card elevation="10">
    <v-card-item>
      <div class="d-md-flex justify-space-between mb-mb-0 mb-3">
        <div>
          <v-card-title class="text-h5">{{ $t("MerchantsUsagePlan") }}</v-card-title>
          <v-card-subtitle class="text-subtitle-1"
            >{{ $t("OverviewOfProfit") }}</v-card-subtitle
          >
        </div>
        <div class="d-flex align-center gap-4">
          <div class="d-flex align-center gap-2">
            <v-avatar size="10" class="bg-primary rounded-circle"></v-avatar>
            <span class="textSecondary">{{ $t("Plus") }}</span>
          </div>
          <div class="d-flex align-center gap-2">
            <v-avatar size="10" class="bg-error rounded-circle"></v-avatar>
            <span class="textSecondary">{{ $t("Pro") }}</span>
          </div>
        </div>
      </div>
      ...
      <v-row class="mt-1">
        <v-col
          v-for="item in RevenueForecastData"
          :key="item.subtext"
          cols="12"
          md="4"
        >
          <div class="d-flex gap-3 align-center">
            ...
            <div>
              <p class="textSecondary">{{ $t(item.subtext) }}</p>
